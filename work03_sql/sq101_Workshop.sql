...
SELECT *
FROM emp
WHERE hiredate BETWEEN '80/1/1' AND '80/12/31';
...
--    총액의 별칭은 Total로 한다.
SELECT ename, sal, sal*comm/100 "커미션 금액", sal+comm "Total"
FROM emp
WHERE comm IS NOT NULL
ORDER BY "Total" DESC;
...
SELECT ename, sal, Round(sal*0.15*10)/10 "회비"
FROM emp
...
FROM emp
ORDER BY sal DESC;
--  sql01_Workshop.sql 이름으로 저장하시고 MM에 업로드 해주시기 바랍니다